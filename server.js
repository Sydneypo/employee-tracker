...
const db = new Database({
    ...
    port: 3306,
    user: "root",
    password: "PatriciaMichael@4588",
    database: "employee_DB"
});

// Builds employee table
...
// Adds a new employoee after asking for name, role, and manager
...
async function removeEmployee() {
    ...
    inquirer.prompt([
        ...
        if (response.employeeName != "Cancel") {
            let deletedEmployee = employees.find(obj => obj.name === response.employeeName);
            db.query("DELETE FROM employee WHERE id=?", deletedEmployee.id);
...
async function updateManager() {
    ...
    inquirer.prompt([
        ...
        inquirer.prompt([
            ...
            let empID = employees.find(obj => obj.name === employeesInfo.empName).id
            ...
            console.log("\x1b[32m", `${employeeInfo.empName} now reports to ${managerInfo.managerName}`);
            runApp();
        })
    })
};
